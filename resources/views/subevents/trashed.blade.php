<x-app-layout>
    ...
    <div class="py-12" x-data="{ showDeleteModal: false, selectedId: null, selectedName: '' }">
                ...
                <div class="p-6">
                    @if($subEvents->isEmpty())
                        <div class="text-center py-12">
                            ...
                            <p class="mt-1 text-sm text-gray-500">No hay reportes eliminados.</p>
                        </div>
                    @else
                                    ...
                                    @foreach($subEvents as $reporte)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                {{ $reporte->deleted_at->diffForHumans() }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex justify-end space-x-2">
                                                    {{-- Restaurar --}}
                                                    <form action="{{ route('subevents.restore', $reporte->id) }}" method="POST">
                                                        @csrf
                                                        <button type="submit" class="text-green-600 hover:text-green-900 bg-green-50 hover:bg-green-100 p-2 rounded-full" title="Restaurar">
                                                    ...
                                                    <button @click="selectedId = {{ $reporte->id }}; selectedName = '{{ addslashes($reporte->report_title) }}'; showDeleteModal = true" class="text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 p-2 rounded-full" title="Eliminar definitivamente">
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                                        </svg>
                                                    </button>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">¿Eliminar permanentemente?</h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500" x-text="'Estás a punto de eliminar definitivamente «' + selectedName + '». Esta acción no se puede deshacer.'"></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <form method="POST" :action="'{{ url('subevents') }}/' + selectedId + '/force-delete'">
                            @csrf
                            @method('DELETE')
